...
fn push_bytes(out: &mut Vec<u8>, data: &[u8]) {
    match data.len() {
        ...
        n if n <= 0x4b => { out.push(n as u8); out.extend_from_slice(data); }
        n if n <= 0xff => { out.push(0x4c); out.push(n as u8); out.extend_from_slice(data); }
        n if n <= 0xffff => {
            out.push(0x4d);
...
fn push_int(out: &mut Vec<u8>, n: i64) {
    ...
    else if n == -1 { out.push(0x4f); }
    else if (1..=16).contains(&n) { out.push(0x50 + n as u8); }
    else { let enc = encode_script_int(n); push_bytes(out, &enc); }
}
...
fn parse_script_asm(s: &str) -> Result<Vec<u8>, String> {
    ...
    while i < chars.len() {
        ...
        if chars[i] == '\'' {
            i += 1;
            let mut lit = Vec::new();
            while i < chars.len() && chars[i] != '\'' { lit.push(chars[i] as u8); i += 1; }
            if i < chars.len() { i += 1; }
        ...
        if let Some(byte) = opcode_byte(&token) { out.push(byte); continue; }

        if let Ok(n) = token.parse::<i64>() { push_int(&mut out, n); continue; }
...
fn parse_prevouts(arr: &Value) -> Result<Vec<Prevout>, String> {
    ...
    for item in items {
        ...
        let vout       = row[1].as_i64().ok_or("vout not integer")? as u32;
        ...
        let _txid = Hash256::from_hex(txid_str)
            .map_err(|e| format!("bad txid '{txid_str}': {e}"))?;
        let spk   = parse_script_asm(spk_asm)
            .map_err(|e| format!("bad scriptPubKey '{spk_asm}': {e}"))?;

        result.push(Prevout {
            txout: TxOut { value: amount, script_pubkey: Script::from_bytes(spk) },
        });
    }
    Ok(result)
}
...
fn check_transaction(tx: &Transaction) -> Result<(), String> {
    if tx.inputs.is_empty()  { return Err("no inputs".into()); }
    if tx.outputs.is_empty() { return Err("no outputs".into()); }
    ...
    for out in &tx.outputs {
        ...
        total_out = total_out.checked_add(out.value)
    ...
    for inp in &tx.inputs {
        ...
        if !seen.insert(key) {
            return Err(format!("duplicate input {}:{}", inp.previous_output.txid, inp.previous_output.vout));
        }
    }
    ...
    if !tx.is_coinbase() {
        for inp in &tx.inputs {
            let is_null = inp.previous_output.txid.0 == [0u8; 32]
...
fn parse_entries(json: &Value) -> Vec<TxTestCase> {
    let arr = match json.as_array() { Some(a) => a, None => return Vec::new() };
    let mut out = Vec::new();
    for entry in arr {
        let row = match entry.as_array() { Some(a) => a, None => continue };
        // Comment entries: ["string"] or [] or single-string array
        if row.is_empty() { continue; }
        // If the first element is a string (not array), it's a comment row
        if row[0].is_string() { continue; }
        // Expect: [[prevouts], tx_hex, flags_str]
        if row.len() < 3 { continue; }
        let prevouts_val = &row[0];
        let tx_hex  = match row[1].as_str() { Some(s) => s.to_string(), None => continue };
        let flags   = match row[2].as_str() { Some(s) => s.to_string(), None => continue };
        ...
        out.push(TxTestCase { label, prevouts, tx_hex, flags_str: flags });
    }
    out
}
...
fn tx_valid_json() {
    ...
    for tc in &cases {
        ...
        let tx = match decode_tx(&tc.tx_hex) {
            Ok(t) => t,
            Err(e) => { failures.push(format!("[DECODE] {}: {e}", tc.label)); continue; }
        };
...
fn tx_invalid_json() {
    ...
    if !false_ok.is_empty() {
        eprintln!("\n=== TX_INVALID FALSE-OK ({}/{}) ===", false_ok.len(), total);
        for f in &false_ok { eprintln!("  {f}"); }
        panic!("{}/{} tx_invalid.json cases falsely accepted", false_ok.len(), total);
    }
